...
# =============================================================================
# PDF FONKSİYONLARI (ORİJİNAL TASARIM GERİ GELDİ)
# =============================================================================
...
with tab_rapor:
    if check_password():
        ...
        if not df_rapor.empty:
            ...
            # --- TOPLU SİLME ALANI ---
            st.markdown("---"); st.header("🗑️ Toplu Veri Silme")
            col_del1, col_del2 = st.columns(2)
            
            with col_del1:
                st.error("⚠️ Müşteriye Göre Sil")
                del_mus = st.selectbox("Hangi Müşterinin Tüm Kayıtları Silinsin?", musteriler)
                if st.button(f"🚨 '{del_mus}' Kişisinin TÜMÜNÜ SİL"):
            ...
            with col_del2:
                ...
                del_tar = st.selectbox("Hangi Tarihteki Tüm Kayıtlar Silinsin?", tarihler)
                if st.button(f"🚨 '{del_tar}' Tarihli TÜMÜNÜ SİL"):
