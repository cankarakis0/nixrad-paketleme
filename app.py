...
def create_thermal_labels_3x6(etiket_listesi, musteri_bilgileri, toplam_etiket_sayisi):
    ...
    # Yazı Stilleri (Kutudan Taşmayı ve Kesilmeyi Önleyen Dinamik Yapı)
    style_sheet = getSampleStyleSheet()
    
    # Alıcı İsmi Stili (Bold ve orta boy)
    style_alici = ParagraphStyle('alici', parent=style_sheet['Normal'], fontSize=7.5, fontName='Helvetica-Bold', leading=8)
    # Adres Stili (Kutu genişliğine göre otomatik alt satıra geçer)
    ...
    for p in etiket_listesi:
        ...
        w, h = p_alici.wrap(width - 4*mm, 10*mm) 
        ...
        aw, ah = p_adres.wrap(width - 4*mm, 12*mm) 
        p_adres.drawOn(c, 2*mm, last_y - ah)

        # --- ALT BÖLÜM (URUN, TEL & DESI) ---
...
with tab_dosya:
    ...
    if st.session_state['ham_veri']:
        ...
        edited_malz_df = st.data_editor(malz_df, key="malzeme_editor", num_rows="dynamic", use_container_width=True)
        ...
        # GÜNCELLENEN TERMAL BUTON VE FONKSİYON ÇAĞRISI
        pdf_thermal = create_thermal_labels_3x6(final_etiket_listesi, musteri_data, int(toplam_parca))
        col_pdf3.download_button(label="🏷️ 3. TERMAL ETIKET (6x3)", data=pdf_thermal, file_name="Nixrad_Termal_Etiket.pdf", mime="application/pdf", use_container_width=True)
...
with tab_manuel:
    ...
    with col_m1:
        ...
        label_1, label_2 = ("Genişlik (cm)", "Yükseklik (cm)") if is_havlupan else ("Yükseklik (cm)", "Genişlik (cm)")
        
    with col_m2: val_1 = st.number_input(label_1, min_value=10, value=50 if is_havlupan else 60)
    with col_m3: val_2 = st.number_input(label_2, min_value=10, value=70 if is_havlupan else 100)
    with col_m4: m_adet = st.number_input("Adet", min_value=1, value=1, key="man_adet")
        
    if st.button("➕ Listeye Ekle", type="primary"):
        g_in, y_in = (val_1, val_2) if is_havlupan else (val_2, val_1)
        birim_desi, boyut_str, birim_kg = manuel_hesapla(secilen_model, g_in, y_in, m_adet)
        st.session_state['manuel_liste'].append({"Model": secilen_model, "Ölçü (ExB)": f"{g_in} x {y_in}", "Kutulu Ölçü": boyut_str, "Adet": m_adet, "Birim Desi": birim_desi, "Toplam Desi": round(birim_desi * m_adet, 2), "Toplam Ağırlık": f"{birim_kg:.2f} KG"})
        st.success("Eklendi!")
    ...
    if st.session_state['manuel_liste']:
        ...
        c_tot1.metric("Toplam Parça", df_manuel['Adet'].sum())
        c_tot2.metric("Genel Toplam Desi", f"{df_manuel['Toplam Desi'].sum():.2f}")
