...
# =============================================================================
# PDF FONKSİYONLARI (YENİ ÖZELLİKLER BURADA)
# =============================================================================
...
with tab_dosya:
            ...
            if header_index != -1:
                ...
                for index, row in df.iterrows():
                    ...
                    if adet > 0:
                        ...
                        is_acc = any(x in stok_lower for x in ['volan', 'tapa', 'aksesuar', 'set'])
    ...
    if st.session_state['ham_veri']:
        ...
        with ozet_alani:
            ...
            st.code(f"toplam desi {p_desi:.2f}", language="text")
        ...
        for _, row in edited_df.iterrows():
            for _ in range(int(row['Adet'])):
                final_etiket.append({'sira_no': len(final_etiket)+1, 'kisa_isim': row['Ürün'], 'boyut_str': row['Ölçü'], 'desi_val': row['Birim Desi']})

        st.divider(); col1, col2, col3 = st.columns(3)
        col1.download_button("📄 1. KARGO FISI (A4)", create_cargo_pdf(p_desi, toplam_parca, musteri_data, edited_df.to_dict('records')), "Kargo_Fisi.pdf", "application/pdf", use_container_width=True)
        col2.download_button("🏭 2. URETIM & ETIKETLER", create_production_pdf(final_malz, final_etiket, musteri_data), "Uretim.pdf", "application/pdf", use_container_width=True)
        col3.download_button("🏷️ 3. TERMAL ETIKET (3x6)", create_thermal_labels_3x6(final_etiket, musteri_data, len(final_etiket)), "Termal.pdf", "application/pdf", use_container_width=True)
...
with tab_manuel:
    ...
    if st.session_state['manuel_liste']:
        ...
        for r in st.session_state['manuel_liste']:
            for _ in range(r['Adet']):
                m_etiket.append({'sira_no': len(m_etiket)+1, 'kisa_isim': r['Ürün'], 'boyut_str': r['Ölçü'], 'desi_val': r['Birim Desi']})
