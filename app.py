...
# --- YENİ EKLENEN ÖZELLİK: 3x6 TERMAL ETİKET ---
def create_thermal_labels_3x6(etiket_listesi, musteri_bilgileri):
    buffer = io.BytesIO()
    label_size = (60*mm, 30*mm) # 6 cm genişlik, 3 cm yükseklik
    doc = SimpleDocTemplate(buffer, pagesize=label_size, rightMargin=1*mm, leftMargin=1*mm, topMargin=1*mm, bottomMargin=1*mm)
    elements = []
    styles = getSampleStyleSheet()
    
    style_no = ParagraphStyle('no', fontSize=8, fontName='Helvetica-Bold', alignment=TA_RIGHT)
    style_model = ParagraphStyle('mod', fontSize=9, fontName='Helvetica-Bold', alignment=TA_CENTER, leading=10)
    style_dim = ParagraphStyle('dim', fontSize=8, alignment=TA_CENTER, leading=9)
    ...
    for p in etiket_listesi:
        ...
        boyut = p['boyut_str']
        desi = str(p['desi_val'])
        cust = tr_clean_for_pdf((musteri_bilgileri['AD_SOYAD'] or "")[:22])

        content = [
            [Paragraph(f"#{no}", style_no)],
            [Paragraph(f"<b>{isim}</b>", style_model)],
            [Paragraph(f"{boyut}", style_dim)],
            [Paragraph(f"Desi: {desi} | {cust}", style_cust)]
        ]
...
with tab_dosya:
    ...
    if st.session_state['ham_veri']:
        ...
        col_pdf2.download_button(label="🏭 2. URETIM & ETIKETLER (A4)", data=pdf_production, file_name="Uretim_ve_Etiketler.pdf", mime="application/pdf", use_container_width=True)

        # --- TERMAL ETİKET BUTONU ---
        pdf_thermal = create_thermal_labels_3x6(final_etiket_listesi, musteri_data)
        col_pdf3.download_button(label="🏷️ 3. TERMAL ETIKET (3x6)", data=pdf_thermal, file_name="Termal_Etiketler.pdf", mime="application/pdf", use_container_width=True)
...
with tab_manuel:
    ...
    with col_m1:
        ...
        val_1_default, val_2_default = (50, 70) if is_havlupan else (60, 100)
    ...
    if st.button("➕ Listeye Ekle", type="primary"):
        ...
        st.session_state['manuel_liste'].append({
            "Model": secilen_model, "Ölçü (ExB)": f"{g_input} x {y_input}",
            "Kutulu Ölçü": boyut_str, "Adet": m_adet, "Birim Desi": birim_desi,
            "Toplam Desi": round(birim_desi * m_adet, 2), "Toplam Ağırlık": f"{birim_kg:.2f} KG"
        })
        st.success("Eklendi!")
    ...
    if st.session_state['manuel_liste']:
        ...
        # Manuel liste için termal etiket basma seçeneği
        m_etiketler = []
        c = 1
