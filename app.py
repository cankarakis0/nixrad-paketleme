...
with tab_manuel:
    ...
    if st.button("➕ Listeye Ekle", type="primary"):
        ...
        b_desi, b_str, b_kg = manuel_hesapla(secilen_model, g, y, m_adet)
        st.session_state['manuel_liste'].append({"Model": secilen_model, "Ölçü (ExB)": f"{g} x {y}", "Kutulu Ölçü": b_str, "Adet": m_adet, "Birim Desi": b_desi, "Toplam Desi": round(b_desi*m_adet, 2), "Toplam Ağırlık": f"{b_kg:.2f} KG"})
    ...
    if st.session_state['manuel_liste']:
        ...
        for _, r in df_m.iterrows():
            for _ in range(int(r['Adet'])):
                m_etiket.append({'sira_no': c, 'kisa_isim': r['Model'], 'boyut_str': r['Kutulu Ölçü'], 'desi_val': r['Birim Desi']})
                c += 1
        
        st.download_button("🏷️ MANUEL TERMAL ETIKET BAS (3x6)", create_thermal_labels_3x6(m_etiket, musteri_data), "Manuel_Termal_Etiketler.pdf", "application/pdf")
